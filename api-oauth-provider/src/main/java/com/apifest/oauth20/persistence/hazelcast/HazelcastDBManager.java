...
public class HazelcastDBManager implements DBManager {
    ...
    public AuthCode findAuthCode(String authCode, String redirectUri) {
        ...
        System.out.println("authCodefind=" + authCode);
//        PersistentAuthCode authStored = getAuthCodeContainer().get(authCode);
//
        AuthCode authStored=null;
        IMap<String, PersistentAuthCode> authContainer = getAuthCodeContainer();
        for (String key : authContainer.keySet()) {
            System.out.println("aaa=" + PersistenceTransformations.toAuthCode(authContainer.get(key)).getCode());
            System.out.println("bbb=" + PersistenceTransformations.toAuthCode(authContainer.get(key)).getRedirectUri());
            System.out.println("ccc=" + PersistenceTransformations.toAuthCode(authContainer.get(key)).getScope());
            System.out.println("vvv=" + PersistenceTransformations.toAuthCode(authContainer.get(key)).getClientId());
            if (authCode.equals(PersistenceTransformations.toAuthCode(authContainer.get(key)).getCode())) {
                authStored=PersistenceTransformations.toAuthCode(authContainer.get(key));

            }
        }
        return authStored;
//        System.out.println("authStored="+authStored);
//        if (values.isEmpty()) {
//            return null;
//        }
